...
from train_models import training_df_model, load_preTrainedModel
...
#%% fair model specific other hyperparameters
burnin_iters = 100
stepSize = 0.1
...
_, _, target_acc, _, _, best_fairness,_,_, _ = evaluate_model(typ_model, devData, dev_S, dev_y, intersectionalGroups)
...
import sys
sys.stdout=open("results/df-model/df_model_report.txt","w")

print(f"Training DF models by grid search over trade-off parameters")
print('\n')
print('\n')
                    ...
                    for miniBatch in mini_batch_sizes:
                        for wd in weight_decays:
                            for ld in lamda_grid:
                                ...
                                df_model = training_df_model(input_size,hiddens,a_func,d_out,lr,num_epochs,trainData,trainLabel,miniBatch,S,intersectionalGroups,burnin_iters,stepSize,epsilonBase,lamda,wd)
                                ...
                                Train_results['SF-hard'][run], Train_results['SF-soft'][run] = evaluate_model(df_model, trainData, S, trainLabel.numpy().reshape((-1,)), intersectionalGroups)
                                ...
                                Dev_results['SF-hard'][run], Dev_results['SF-soft'][run] = evaluate_model(df_model, devData, dev_S, dev_y, intersectionalGroups)
                                ...
                                Test_results['SF-hard'][run], Test_results['SF-soft'][run] = evaluate_model(df_model, testData, test_S, test_y, intersectionalGroups)
                                ...
                                if Dev_results['accuracy'][run]>=target_acc and Dev_results['DF-hard'][run]<best_fairness:
                                    best_fairness = Dev_results['DF-hard'][run]
                                    print('\n')
                                    print(f"Current best model:")
                                    print('hidden layers (nodes/layer): ', hiddens, 'learning rate: ', lr, 'Drop out prob: ', d_out, 'mini-batch sizes: ', miniBatch, 'activation function: ', a_func, 'weight decay: ', wd, 'lambda: ',ld)
                                    print(f"accuracy on dev set: {Dev_results['accuracy'][run]: .4f}")
                                    print(f"df (hard) on dev set: {Dev_results['DF-hard'][run]: .4f}")
                                    best_net = df_model
                                    print('\n')
                                    print('\n')
...
Train_results.to_csv('results/df-model/Train_results.csv',index=False)
Dev_results.to_csv('results/df-model/Dev_results.csv',index=False)
Test_results.to_csv('results/df-model/Test_results.csv',index=False)

#%% saving model and outcomes

torch.save(best_net.state_dict(), "trained-models/DF_Model")

np.savetxt('results/df-model/pred_prob_test.txt',pred_prob_test)
